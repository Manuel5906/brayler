...
export const getWAUploadToServer = (
	...
	return async (filePath, { mediaType, fileEncSha256B64, timeoutMs }) => {
		...
		const getHeadersObject = () => {
			...
			if (!hdrs) return {}
			if (Array.isArray(hdrs)) return Object.fromEntries(hdrs)
			return hdrs as Record<string, string>
		}
		...
		for (const { hostname, maxContentLengthBytes } of hosts) {
			...
			try {
				...
				// Refresh media connection after each failure for fresh auth token
				if (!isLast) {
					try {
						uploadInfo = await refreshMediaConn(true)
					} catch (refreshError) {
						logger.warn({ error: refreshError }, 'Failed to refresh media connection')
					}
				}
			}
		}
