...
export const generateWAMessageContent = async (
	...
	if (hasNonNullishProperty(message, 'text')) {
		...
		// Handle interactive message with native flow buttons
	...
	if (hasOptionalProperty(message, 'buttons') && message.buttons?.length) {
		...
		if (hasNonNullishProperty(message, 'text')) {
			...
			const type = Object.keys(m)[0]?.replace('Message', '').toUpperCase() as keyof typeof ButtonType
	...
	// Handle template buttons message
	else if (hasOptionalProperty(message, 'templateButtons') && message.templateButtons?.length) {
	...
	// Handle list message
	if (hasOptionalProperty(message, 'sections') && message.sections?.length) {
		const listMessage: proto.Message.IListMessage = {
			sections: message.sections,
			buttonText: (message as any).buttonText,
			title: (message as any).title,
			footerText: (message as any).footer,
			description: hasNonNullishProperty(message, 'text') ? message.text : '',
